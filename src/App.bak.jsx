    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-6xl mx-auto p-4 space-y-4">
        ...
        {tab === "welcome" && (
          <section className="bg-white border rounded-xl p-6 space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-700">
              ...
              <div>
                <h4 className="font-medium">Quick start (2â€“3 minutes)</h4>
        ...
        <footer className="py-8 text-xs text-gray-500">
          <p>Â© {new Date().getFullYear()} Financial Organizer. This software provides general information only and is not legal, tax, or financial advice. Laws vary by jurisdiction. Consult a qualified professional.</p>
        </footer>
      </main>
    </div>
  );
...
# 1) Add imports directly under jspdf-autotable
src = src.replace(
    'import "jspdf-autotable";',
    'import "jspdf-autotable";\nimport DivorceTab from "./components/DivorceTab.jsx";\nimport { ensureDivorceDefaults } from "./lib/divorceDefaults.js";'
)

# 2) Wrap usePersistentState initializer with ensureDivorceDefaults(...)
src = src.replace(
    """if (!raw) return initial;
      return JSON.parse(raw);
    } catch { return initial; }""",
...
# 3) Insert the Divorce tab button right before Settings
src = src.replace(
    '          <TabBtn id="settings" label="Settings" />',
    '          <TabBtn id="divorce" label="Divorce" />\n          <TabBtn id="settings" label="Settings" />',
)

# 4) Add Divorce tab content before Settings section
divorce_section = """
        {tab === "divorce" && (
          <section className="bg-white border rounded-xl p-6 space-y-4">
            <DivorceTab
              ...
              // parseCSV={parseCSV} // uncomment if you want to reuse your CSV parser
            />
          </section>
        )}
"""
...
# 5) Insert PDF helper call before doc.save(...)
src = src.replace(
    'doc.save(`financial-report-${new Date().toISOString().slice(0,10)}.pdf`);',
    'pdfAddDivorce(doc, data);\n\n    doc.save(`financial-report-${new Date().toISOString().slice(0,10)}.pdf`);'
)

# 6) Append the helper function below generatePDF (place it right after generatePDF block)
pdf_helper = """
  // (Optional) Divorce PDF helper
  function pdfAddDivorce(doc, data) {
    if (!data?.divorce) return;
    const d = data.divorce;
    ...
    doc.text(`Children: ${typeof d.children === "number" ? d.children : "-"}`, 40, 90);

    const contacts = (d.attorneyContacts || []).map(c => [
      c.name || "", c.role || "", c.email || "", c.phone || ""
    ]);
    if (contacts.length) {
      // @ts-ignore
      doc.autoTable({ head: [["Name","Role","Email","Phone"]], body: contacts, startY: 110, styles: { fontSize: 9 } });
    }
    const y1 = (doc.lastAutoTable?.finalY ?? 110) + 10;

    const deadlines = (d.deadlines || []).map(x => [x.label || "", x.dateISO || "", x.done ? "Yes" : "No"]);
    // @ts-ignore
    doc.autoTable({ head: [["Deadline","Date","Done"]], body: deadlines, startY: y1, styles: { fontSize: 9 } });
    const y2 = (doc.lastAutoTable?.finalY ?? y1) + 10;

    const disclosures = (d.disclosures || []).map(x => [x.label || "", x.provided ? "Yes" : "No"]);
    // @ts-ignore
    doc.autoTable({ head: [["Disclosure","Provided"]], body: disclosures, startY: y2, styles: { fontSize: 9 } });
    const y3 = (doc.lastAutoTable?.finalY ?? y2) + 10;

    const a = d.support?.requestedAlimonyMonthly ?? 0;
    const c = d.support?.requestedChildSupportMonthly ?? 0;
    const s = d.support?.startDateISO || "-";
    // @ts-ignore
    doc.autoTable({
    ...
    doc.setFontSize(8);
    doc.text("Informational only â€” not legal advice.", 40, (doc.lastAutoTable?.finalY ?? y3) + 12);
  }
"""
# Insert helper after the end of generatePDF definition
src = src.replace(
    "  };",
    "  };" + pdf_helper,
    1  # only the first occurrence inside App()
)

# 7) Tweak Danger Zone Reset button
src = src.replace(
    "if (confirm('Clear all local data?')) setData(defaultState);",
    "if (confirm('Clear all local data?')) setData(ensureDivorceDefaults(defaultState));"
)

# 8) Update the usePersistentState initializer RETURN wrappers require ensureDivorceDefaults import - already added.

# 9) Also update the top-level default initialization via ensureDivorceDefaults when reading from storage - already done.
# Additionally, ensure the very first load of state includes divorce defaults when no storage:
# (Handled by step 2)


# Save outputs
out_path = Path("/mnt/data/App.jsx")
out_path.write_text(src, encoding="utf-8")

# Make a timestamped copy and a ZIP for convenience
ts = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
copy_path = Path(f"/mnt/data/App.divorce-wired-{ts}.jsx")
copy_path.write_text(src, encoding="utf-8")

zip_path = Path(f"/mnt/data/App.divorce-wired-{ts}.zip")
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
    zf.write(out_path, arcname="App.jsx")
    zf.write(copy_path, arcname=copy_path.name)
